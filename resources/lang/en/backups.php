<?php

return [
    'navigation' => [
        'group' => 'Backups',
    ],

    'pages' => [
        'dashboard' => [
            'navigation_label' => 'Overview',
            'title' => 'Backups',
            'actions' => [
                'run_backup' => [
                    'label' => 'Create snapshot',
                    'modal_heading' => 'Create snapshot',
                    'modal_description' => 'This will start a backup job and create a new snapshot.',
                ],
                'open_runs' => 'Open Runs',
                'open_snapshots' => 'Open Snapshots',
                'open_settings' => 'Open Settings',
            ],
            'notifications' => [
                'operation_in_progress' => 'Operation in progress',
                'operation_running' => 'Another operation is running.',
                'operation_running_type' => 'Type: :type.',
                'operation_running_run_id' => 'Run ID: :run_id.',
                'backup_waits' => 'Backup will wait in queue.',
                'backup_queued' => 'Backup queued',
                'backup_queued_body' => 'Backup job has been queued and will run in background.',
            ],
            'header_actions' => [
                'refresh' => 'Refresh',
            ],
        ],
        'settings' => [
            'navigation_label' => 'Settings',
            'title' => 'Backup Settings',
            'sections' => [
                'repository_status' => [
                    'title' => 'Repository status',
                    'status' => 'Status: :status',
                    'snapshots' => 'Snapshots: :count',
                    'refresh' => 'Refresh status',
                    'init' => [
                        'label' => 'Init repository',
                        'modal_heading' => 'Init repository',
                        'modal_description' => 'This will initialize a new restic repository at the computed path.',
                        'confirm_label' => 'Type INIT to confirm',
                        'recompute_path_label' => 'Recompute path from APP_NAME/APP_ENV',
                        'recompute_path_helper' => 'Before initialization, updates the restic/<app>/<env> path from current config values.',
                    ],
                ],
                'storage' => [
                    'title' => 'Storage (S3)',
                    'description' => 'Configure S3-compatible storage for restic.',
                    'endpoint' => [
                        'label' => 'Endpoint',
                        'placeholder' => 'https://s3.example.com',
                        'helper' => 'Scheme is optional; https:// will be used by default.',
                    ],
                    'bucket_select' => [
                        'label' => 'Bucket',
                        'placeholder' => 'Refresh to load buckets',
                    ],
                    'bucket' => [
                        'label' => 'Bucket',
                        'placeholder' => 'Enter bucket name',
                        'helper' => 'Enter manually if listing is not available.',
                    ],
                    'refresh_buckets' => 'Refresh buckets',
                    'access_key' => 'Access key',
                    'secret_key' => 'Secret key',
                ],
                'repository' => [
                    'title' => 'Repository',
                    'path_label' => 'Repository path',
                    'path_helper' => 'Auto-generated: restic/<app>/<env>.',
                    'uri_label' => 'Repository URI',
                    'password_label' => 'Repository password',
                    'password_helper' => 'Required on first setup. Store it safely.',
                ],
                'retention' => [
                    'title' => 'Retention policy',
                    'description' => 'If empty or 0, retention may be skipped.',
                    'keep_last' => 'Keep last',
                    'keep_daily' => 'Keep daily',
                    'keep_weekly' => 'Keep weekly',
                    'keep_monthly' => 'Keep monthly',
                    'keep_yearly' => 'Keep yearly',
                ],
                'schedule' => [
                    'title' => 'Schedule',
                    'enabled' => 'Enable schedule',
                    'daily_time' => 'Daily time',
                    'timezone' => 'Timezone',
                ],
                'paths' => [
                    'title' => 'Paths',
                    'description' => 'If include is empty, restic backs up the entire project root, excluding the paths below.',
                    'project_root_label' => 'Project root',
                    'project_root_helper' => 'Always uses base_path().',
                    'include_label' => 'Include paths',
                    'include_helper' => 'Leave empty to back up the whole project root. Use relative paths.',
                    'include_placeholder' => 'storage/app',
                    'exclude_label' => 'Exclude paths',
                    'exclude_helper' => 'Do not exclude storage/app/_backup, or the database dump will be missing.',
                    'exclude_placeholder' => 'storage/logs',
                    'storage_symlink' => [
                        'label' => 'Storage is a shared symlink',
                        'helper' => 'When enabled, backups include the target contents of the storage symlink. Downloaded archives place it into _shared/storage.',
                        'status_resolved' => 'The storage path is a symlink and its target was resolved successfully.',
                        'status_not_symlink' => 'The storage path exists, but it is not a symlink.',
                        'status_unresolved_target' => 'The storage symlink exists, but its target could not be resolved.',
                        'status_missing' => 'The storage path was not found under the project root.',
                        'target' => 'Resolved target: :path',
                        'restore_notice' => 'Automatic restore does not write _shared/storage back. Restore this directory manually into the storage symlink target.',
                    ],
                    'recommended_defaults' => 'Apply recommended excludes',
                ],
            ],
            'actions' => [
                'save' => 'Save',
                'run_backup' => [
                    'label' => 'Create snapshot',
                    'modal_heading' => 'Create snapshot',
                    'modal_description' => 'This will start a backup job and create a new snapshot.',
                ],
                'reset_settings' => [
                    'label' => 'Reset settings',
                    'modal_heading' => 'Reset backup settings',
                    'modal_description' => 'This clears the saved repository configuration and restores the plugin defaults. Existing runs and snapshots are not deleted.',
                    'confirm_label' => 'Type RESET to confirm',
                    'password_label' => 'Current password',
                ],
            ],
            'notifications' => [
                'settings_saved' => 'Settings saved',
                'settings_reset' => 'Settings reset',
                'operation_in_progress' => 'Operation in progress',
                'operation_running' => 'Another operation is running.',
                'operation_running_type' => 'Type: :type.',
                'operation_running_run_id' => 'Run ID: :run_id.',
                'backup_waits' => 'Backup will wait in queue.',
                'backup_queued' => 'Backup queued',
                'backup_queued_body' => 'Backup job has been queued and will run in background.',
                'missing_credentials' => 'Missing credentials',
                'missing_credentials_body' => 'Endpoint, access key, and secret key are required to list buckets.',
                'bucket_listing_failed' => 'Bucket listing failed',
                'bucket_listing_failed_body' => 'Failed to list buckets.',
                'buckets_refreshed' => 'Buckets refreshed',
                'confirmation_mismatch' => 'Confirmation mismatch',
                'confirmation_mismatch_body' => 'Type INIT to confirm repository initialization.',
                'reset_confirmation_mismatch' => 'Confirmation mismatch',
                'reset_confirmation_mismatch_body' => 'Type RESET to confirm settings reset.',
                'reset_password_invalid' => 'Password check failed',
                'reset_password_invalid_body' => 'The current password does not match your account.',
                'reset_password_unavailable' => 'Password check unavailable',
                'reset_password_unavailable_body' => 'The current account cannot be verified with a password.',
                'repository_not_configured' => 'Repository not configured',
                'repository_not_configured_body' => 'Fill endpoint, bucket, keys, and repository password first.',
                'init_failed' => 'Init failed',
                'repository_initialized' => 'Repository initialized',
            ],
            'repository_status' => [
                'labels' => [
                    'available' => 'Available',
                    'not_initialized' => 'Not initialized',
                    'not_configured' => 'Not configured',
                    'error' => 'Error',
                ],
                'messages' => [
                    'not_configured' => 'Repository is not configured yet.',
                    'available' => 'Repository is available.',
                    'not_initialized' => 'Repository not initialized.',
                ],
            ],
            'errors' => [
                'unknown_error' => 'Unknown error.',
            ],
            'placeholders' => [
                'not_available' => 'n/a',
            ],
        ],
        'exports' => [
            'navigation_label' => 'Recovery Exports',
            'title' => 'Recovery Exports',
            'actions' => [
                'full' => [
                    'label' => 'Download FULL (Complete archive)',
                    'modal_heading' => 'Create FULL export',
                    'modal_description' => 'Checks free disk space before creating the archive.',
                    'modal_submit_label' => 'Start FULL export',
                ],
                'delta' => [
                    'label' => 'Download DELTA (Since last FULL)',
                    'modal_heading' => 'Create DELTA export',
                    'modal_description' => 'Checks free disk space before creating the archive.',
                    'modal_submit_label' => 'Start DELTA export',
                ],
            ],
            'preflight' => [
                'title' => 'Disk space check',
                'available' => 'Available on export disk: :bytes',
                'restore_size' => 'Estimated restored data: :bytes',
                'required' => 'Required temporary disk space: :bytes',
                'missing' => 'Missing: :bytes',
                'source' => 'Estimate source: :source',
                'estimate_unavailable' => 'The required disk space could not be estimated safely.',
                'result_ok' => 'Enough disk space for this export.',
                'result_fail' => 'Not enough disk space for this export.',
            ],
            'sections' => [
                'baseline' => [
                    'title' => 'Baseline (FULL)',
                    'description' => 'Baseline snapshot used for delta exports.',
                    'snapshot' => 'Baseline snapshot: :id',
                    'created_at' => 'Baseline created at: :time',
                    'status' => 'Status: :status',
                ],
                'latest' => [
                    'title' => 'Latest snapshot',
                    'description' => 'Exports always use the latest snapshot.',
                    'snapshot' => 'Latest snapshot: :id',
                    'time' => 'Time: :time',
                    'ok' => 'Snapshot looks ready for export.',
                    'warn_excludes' => 'Warning: latest snapshot excludes vendor or public/build.',
                ],
                'downloads' => [
                    'title' => 'Download links',
                    'description' => 'Latest ready FULL and DELTA archives.',
                    'kind' => [
                        'full' => 'FULL',
                        'delta' => 'DELTA',
                    ],
                    'line_not_ready' => ':kind: n/a',
                    'line_no_changes' => ':kind: no changes relative to FULL, no download link needed.',
                    'download' => 'Download',
                    'run' => 'run #:run_id',
                ],
            ],
            'status' => [
                'available' => 'Available',
                'missing' => 'Missing in repository',
            ],
            'placeholders' => [
                'not_set' => 'not set',
                'not_available' => 'not available',
            ],
            'notifications' => [
                'snapshot_missing' => 'Latest snapshot not found.',
                'baseline_missing' => 'Baseline snapshot is missing.',
                'operation_in_progress' => 'Operation in progress',
                'operation_running' => 'Another operation is running.',
                'export_disk_space_insufficient' => 'Export cancelled before start',
                'export_disk_space_insufficient_body' => 'Available: :available. Required: :required. Missing: :missing.',
                'export_disk_space_unknown_body' => 'The export was not started because the required disk space could not be estimated safely.',
                'full_queued' => 'FULL export queued',
                'full_queued_body' => 'FULL export job was queued and will run in the background.',
                'delta_queued' => 'DELTA export queued',
                'delta_queued_body' => 'DELTA export job was queued and will run in the background.',
            ],
            'errors' => [
                'restic_not_configured' => 'Restic repository is not configured.',
                'unable_to_load' => 'Unable to load snapshots.',
                'unable_to_load_from_restic' => 'Unable to load snapshots from restic.',
            ],
            'header_actions' => [
                'refresh' => 'Refresh',
            ],
        ],
        'snapshots' => [
            'navigation_label' => 'Snapshots',
            'title' => 'Snapshots',
            'table' => [
                'columns' => [
                    'id' => 'ID',
                    'time' => 'Time',
                    'size' => 'Data size (disk)',
                    'host' => 'Host',
                    'tags' => 'Tags',
                    'archive' => 'Archive',
                    'paths' => 'Paths',
                ],
                'filters' => [
                    'tag' => 'Tag',
                    'host' => 'Host',
                    'date_range' => 'Date range',
                    'from' => 'From',
                    'until' => 'Until',
                ],
            ],
            'actions' => [
                'delete' => [
                    'label' => 'Delete',
                    'modal_heading' => 'Delete snapshot',
                    'modal_description' => 'This will remove the snapshot from the repository. Prune may take some time.',
                    'confirm_label' => 'Confirm snapshot ID',
                    'confirm_help' => 'Type the first 8 characters: :short_id',
                ],
                'restore' => [
                    'label' => 'Restore...',
                    'modal_heading' => 'Restore snapshot',
                    'modal_submit_label' => 'Queue restore',
                ],
                'export' => [
                    'label' => 'Download archive...',
                    'modal_heading' => 'Download snapshot as archive',
                    'modal_description' => 'Creates an archive on the server (restic restore + tar.gz). A download link will appear in the table once it is ready.',
                    'modal_submit_label' => 'Start archive export',
                    'include_env_label' => 'Include .env (unsafe)',
                    'include_env_help' => 'Disabled by default for security. Enable only if you really need it.',
                    'keep_hours_label' => 'Keep archive (hours)',
                    'preflight' => [
                        'title' => 'Disk space check',
                        'available' => 'Available on export disk: :bytes',
                        'restore_size' => 'Estimated restored data: :bytes',
                        'required' => 'Required temporary disk space: :bytes',
                        'missing' => 'Missing: :bytes',
                        'source' => 'Estimate source: :source',
                        'estimate_unavailable' => 'The required disk space could not be estimated safely.',
                        'result_ok' => 'Enough disk space for this export.',
                        'result_fail' => 'Not enough disk space for this export.',
                    ],
                ],
                'details' => [
                    'label' => 'Details',
                    'modal_heading' => 'Snapshot details',
                ],
            ],
            'archive' => [
                'status' => [
                    'ready' => 'Ready',
                    'queue' => 'In queue',
                    'failed' => 'Failed',
                    'expired' => 'Expired',
                    'deleted' => 'Deleted',
                    'none' => 'n/a',
                ],
                'kind' => [
                    'full' => 'FULL',
                ],
                'size' => 'Archive size: :size',
                'size_help' => 'Archive is a compressed tar.gz of a single snapshot. Data size shows uncompressed files, so the archive can be much smaller.',
                'download' => 'Download',
                'delete' => 'Delete',
                'delete_confirm' => 'Delete this archive?',
            ],
            'notifications' => [
                'snapshot_id_missing' => 'Snapshot ID missing',
                'snapshot_id_missing_delete' => 'Unable to determine snapshot ID for deletion.',
                'snapshot_id_missing_restore' => 'Unable to determine snapshot ID for restore.',
                'snapshot_id_missing_export' => 'Unable to determine snapshot ID for export.',
                'confirmation_mismatch' => 'Confirmation mismatch',
                'confirmation_mismatch_body' => 'Type the first 8 characters of the snapshot ID to confirm.',
                'confirmation_phrase_mismatch' => 'Confirmation phrase mismatch',
                'confirmation_phrase_mismatch_body' => 'Type the exact phrase shown in the wizard to confirm the restore.',
                'preflight_failed' => 'Preflight failed',
                'preflight_failed_body' => 'Insufficient disk space or filesystem mismatch for atomic restore.',
                'operation_in_progress' => 'Operation in progress',
                'operation_running' => 'Another operation is running.',
                'operation_running_type' => 'Type: :type.',
                'operation_running_run_id' => 'Run ID: :run_id.',
                'backup_waits' => 'Backup will wait in queue.',
                'delete_waits' => 'Delete will wait in queue.',
                'restore_waits' => 'Restore will wait in queue.',
                'export_waits' => 'Export will wait in queue.',
                'snapshot_create_queued' => 'Snapshot queued',
                'snapshot_create_queued_body' => 'Backup job has been queued and will create a new snapshot.',
                'snapshot_delete_queued' => 'Snapshot delete queued',
                'snapshot_delete_queued_body' => 'Delete job has been queued and will run in background.',
                'restore_queued' => 'Restore queued',
                'restore_queued_body' => 'Restore job has been queued. During cutover the site will briefly enter maintenance. Use the bypass path from Runs if needed.',
                'restore_ready' => 'Restore completed',
                'restore_ready_body' => 'Restore for snapshot :snapshot completed successfully.',
                'restore_ready_body_generic' => 'Restore completed successfully.',
                'export_queued' => 'Export queued',
                'export_queued_body' => 'Archive export job queued. Open Runs to download when it finishes.',
                'export_disk_space_insufficient' => 'Export cancelled before start',
                'export_disk_space_insufficient_body' => 'Available: :available. Required: :required. Missing: :missing.',
                'export_disk_space_unknown_body' => 'The export was not started because the required disk space could not be estimated safely.',
                'export_ready' => 'Archive ready',
                'export_ready_body' => 'Archive for snapshot :snapshot is ready to download.',
                'snapshot_ready' => 'Snapshot ready',
                'snapshot_ready_body' => 'Snapshot :snapshot is ready.',
                'snapshot_ready_body_generic' => 'Snapshot is ready.',
                'snapshots_refreshed' => 'Snapshots refreshed',
            ],
            'sections' => [
                'operation_in_progress' => [
                    'title' => 'Operation: :type',
                    'description' => 'Running. This may take some time — please be patient.',
                    'type' => 'Type: :type',
                    'run_id' => 'Run ID: :run_id',
                    'started_at' => 'Started: :started_at',
                    'heartbeat' => 'Heartbeat: :heartbeat',
                    'host' => 'Host: :host',
                    'step' => 'Step: :step',
                    'last_activity' => 'Last activity: :activity',
                    'stale_warning' => 'Warning: lock heartbeat is stale.',
                ],
                'repository_issue' => [
                    'title' => 'Repository issue',
                    'description' => 'Unable to load snapshots from restic.',
                    'locked_title' => 'Repository is busy',
                    'locked_description' => 'Restic holds an exclusive lock while operations are running.',
                    'unknown_error' => 'Unknown error',
                    'open_settings' => 'Open Settings',
                ],
            ],
            'operation_types' => [
                'backup' => 'Create snapshot',
                'export_snapshot' => 'Export snapshot',
                'export_full' => 'Export FULL',
                'export_delta' => 'Export DELTA',
                'forget_snapshot' => 'Delete snapshot',
                'restore' => 'Restore',
            ],
            'operation_steps' => [
                'dump' => 'Database dump',
                'restic_backup' => 'Restic backup',
                'retention' => 'Retention cleanup',
                'restic_diff' => 'Restic diff',
                'restic_restore' => 'Download from storage',
                'pack_tar_gz' => 'Pack archive',
                'forget_prune' => 'Forget + prune',
                'preflight' => 'Preflight',
                'preflight_fs' => 'Filesystem preflight',
                'preflight_space' => 'Disk space check',
                'preflight_rsync_write' => 'Rsync write preflight',
                'dump_db_from_snapshot' => 'Dump DB from snapshot',
                'safety_backup' => 'Safety backup',
                'cutover_down' => 'Maintenance mode',
                'cutover_db_wipe' => 'Wipe database',
                'cutover_db_import' => 'Import database',
                'cutover_optimize_clear' => 'Optimize and clear cache',
                'cutover_queue_restart' => 'Restart queue',
                'cutover_up' => 'Bring site up',
                'stage_restic_restore' => 'Stage restic restore',
                'stage_validate' => 'Stage validation',
                'cutover_swap' => 'Swap releases',
                'cutover_env_preserve' => 'Preserve .env',
                'cutover_runtime_prepare' => 'Prepare runtime directories',
                'cutover_down_after_swap' => 'Maintenance after swap',
                'runtime_cleanup' => 'Runtime cleanup',
                'storage_link' => 'Storage link',
                'rollback' => 'Rollback',
            ],
            'header_actions' => [
                'create_snapshot' => 'Create snapshot',
                'create_snapshot_modal_heading' => 'Create snapshot',
                'create_snapshot_modal_description' => 'This will run a backup and create a new snapshot.',
                'refresh' => 'Refresh',
                'settings' => 'Settings',
            ],
            'errors' => [
                'restic_not_configured' => 'Restic repository is not configured.',
                'unable_to_load' => 'Unable to load snapshots.',
                'unable_to_load_from_restic' => 'Unable to load snapshots from restic.',
                'repository_locked' => 'Repository is locked by a running operation.',
                'repository_locked_details' => 'Snapshots will appear after it finishes. If it is stuck, run `php artisan restic-backups:unlock` or `restic unlock`.',
                'exit_code' => 'Exit code: :code.',
                'restic_exit_code' => 'Restic exited with code :code.',
                'truncated' => '...[truncated]',
            ],
            'wizard' => [
                'steps' => [
                    'snapshot' => 'Snapshot',
                    'scope' => 'Scope',
                    'preflight' => 'Preflight',
                    'confirmation' => 'Confirmation',
                ],
                'snapshot' => [
                    'id' => 'Snapshot: :id',
                    'time' => 'Time: :time',
                    'host' => 'Host: :host',
                    'tags' => 'Tags: :tags',
                    'paths' => 'Paths: :paths',
                    'warning' => 'Warning: Restore is a destructive operation.',
                ],
                'scope' => [
                    'label' => 'Restore scope',
                    'helper' => 'DB-only extracts only the database dump; project files are not restored.',
                    'options' => [
                        'files' => 'Files only',
                        'db' => 'Database only',
                        'both' => 'Files + Database',
                    ],
                    'mode_label' => 'Files restore mode',
                    'mode_options' => [
                        'rsync' => 'Rsync (sync into existing project)',
                        'atomic' => 'Atomic swap (new directory, then replace)',
                    ],
                    'mode_helper' => 'Atomic swap minimizes downtime.',
                    'safety_backup' => 'Create safety backup before restore',
                ],
                'preflight' => [
                    'free' => 'Free: :bytes',
                    'estimated' => 'Estimated restore size: :bytes (:source)',
                    'estimated_no_source' => 'Estimated restore size: :bytes',
                    'required' => 'Required (with buffer): :bytes',
                    'result' => 'Result: :result',
                    'result_ok' => 'OK',
                    'result_fail' => 'FAIL',
                    'same_fs_ok' => ' / same FS: OK',
                    'same_fs_fail' => ' / same FS: FAIL',
                    'staging_parent_ok' => ' / staging parent writable: OK',
                    'staging_parent_fail' => ' / staging parent writable: FAIL',
                ],
                'confirmation' => [
                    'phrase_label' => 'Confirmation phrase',
                    'input_label' => 'Type the phrase to confirm',
                    'input_helper' => 'Type the exact phrase shown above.',
                    'phrase_value' => 'RESTORE :app :snapshot :scope',
                    'scope' => [
                        'files' => 'files',
                        'db' => 'db',
                        'both' => 'both',
                    ],
                ],
            ],
            'placeholders' => [
                'not_available' => 'n/a',
                'unknown' => 'unknown',
            ],
        ],
        'runs' => [
            'navigation_label' => 'Runs',
            'title' => 'Backup Runs',
            'header_actions' => [
                'create_snapshot' => 'Create snapshot',
                'create_snapshot_modal_heading' => 'Create snapshot',
                'create_snapshot_modal_description' => 'This will start a backup job and create a new snapshot.',
            ],
            'table' => [
                'columns' => [
                    'id' => 'Run ID',
                    'started' => 'Started',
                    'finished' => 'Finished',
                    'type' => 'Type',
                    'status' => 'Status',
                    'duration' => 'Duration',
                    'trigger' => 'Trigger',
                    'tags' => 'Tags',
                    'exit_code' => 'Exit',
                ],
                'filters' => [
                    'status' => 'Status',
                    'status_options' => [
                        'running' => 'Running',
                        'success' => 'Success',
                        'failed' => 'Failed',
                        'skipped' => 'Skipped',
                    ],
                    'type' => 'Type',
                    'type_options' => [
                        'backup' => 'Backup',
                        'check' => 'Check',
                        'forget' => 'Retention',
                        'forget_snapshot' => 'Delete snapshot',
                        'restore' => 'Restore',
                        'export_snapshot' => 'Export snapshot',
                        'export_full' => 'Export FULL',
                        'export_delta' => 'Export DELTA',
                    ],
                    'trigger_options' => [
                        'manual' => 'Manual',
                        'schedule' => 'Schedule',
                        'system' => 'System',
                        'filament' => 'Filament',
                    ],
                    'started_at' => 'Started date',
                    'from' => 'From',
                    'until' => 'Until',
                ],
            ],
            'actions' => [
                'view' => [
                    'label' => 'View',
                    'modal_heading' => 'Backup run details',
                ],
                'download' => [
                    'label' => 'Download',
                ],
            ],
            'duration' => [
                'seconds' => ':seconds s',
                'minutes' => ':minutes m :seconds s',
                'hours' => ':hours h :minutes m',
            ],
        ],
    ],
    'views' => [
        'placeholders' => [
            'not_available' => 'n/a',
            'dash' => '-',
        ],
        'values' => [
            'yes' => 'yes',
            'no' => 'no',
            'enabled' => 'enabled',
            'disabled' => 'disabled',
        ],
        'scope_values' => [
            'files' => 'Files',
            'db' => 'Database',
            'both' => 'Database + files',
        ],
        'runs' => [
            'labels' => [
                'status' => 'Status',
                'type' => 'Type',
                'started' => 'Started',
                'finished' => 'Finished',
                'duration' => 'Duration',
                'trigger' => 'Trigger',
                'tags' => 'Tags',
                'error' => 'Error',
                'step' => 'Step',
                'restore' => 'Restore',
                'snapshot' => 'Snapshot',
                'scope' => 'Scope',
                'mode' => 'Mode',
                'safety_backup' => 'Safety backup',
                'rollback_path' => 'Rollback path',
                'rollback_path_moved_back' => 'Rollback directory was moved back to project root (:path) during rollback.',
                'safety_dump' => 'Safety DB dump',
                'bypass_path' => 'Bypass path',
                'bypass_notice' => 'If the site returns 503, it is available at:',
                'copy' => 'Copy',
                'db_dump' => 'DB Dump',
                'backup' => 'Backup',
                'retention' => 'Retention',
                'exit' => 'Exit',
                'size' => 'Size',
                'skipped' => 'Skipped (:reason)',
            ],
            'details' => [
                'dump_stderr' => 'Dump stderr',
                'backup_stdout' => 'Backup stdout',
                'backup_stderr' => 'Backup stderr',
                'retention_stdout' => 'Retention stdout',
                'retention_stderr' => 'Retention stderr',
                'restore_steps_raw' => 'Restore steps (raw)',
            ],
        ],
        'overview' => [
            'labels' => [
                'repository' => 'Repository',
                'system' => 'System',
                'snapshots' => 'Snapshots',
                'last' => 'Last',
                'tags' => 'Tags',
                'free_disk' => 'Free disk',
                'queue' => 'Queue',
                'settings_configured' => 'Settings configured',
                'schedule' => 'Schedule',
                'operation' => 'Operation',
                'run_id' => 'Run ID',
                'started' => 'Started',
                'heartbeat' => 'Heartbeat',
                'host' => 'Host',
                'step' => 'Step',
                'last_backup' => 'Last backup',
                'last_restore' => 'Last restore',
                'last_failed' => 'Last failed',
                'duration' => 'Duration',
                'trigger' => 'Trigger',
                'scope' => 'Scope',
            ],
            'status' => [
                'success' => 'Success',
                'failed' => 'Failed',
                'running' => 'Running',
                'skipped' => 'Skipped',
                'unknown' => 'n/a',
                'repo_available' => 'Available',
                'repo_uninitialized' => 'Not initialized',
                'repo_error' => 'Error',
                'lock_likely' => 'Likely locked',
                'lock_not' => 'Not locked',
                'lock_unknown' => 'Unknown',
            ],
            'messages' => [
                'queue_sync' => 'Queue is sync (jobs run inline).',
                'lock_stale' => 'Lock looks stale (no heartbeat recently).',
            ],
        ],
    ],
    'export_space_sources' => [
        'restic_stats' => 'restic restore-size',
        'restic_diff' => 'restic diff statistics',
        'restic_stats_fallback' => 'restic restore-size fallback',
    ],
];
